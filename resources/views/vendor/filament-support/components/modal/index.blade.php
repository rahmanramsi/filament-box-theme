...
<div x-data="{ isOpen: false }"
    ...
    role="dialog" aria-modal="true" class="{{ $displayClasses }} filament-modal">
    {{ $trigger }}
    ...
    <div x-show="isOpen" x-transition:enter="ease duration-300" x-transition:enter-start="opacity-0"
        ...
        <div @if (filled($id)) x-on:click="$dispatch('{{ $closeEventName }}', { id: '{{ $id }}' })"
            @else
                x-on:click="isOpen = false" @endif
            aria-hidden="true"
            class="fixed inset-0 w-full h-full bg-black/50 cursor-pointer filament-modal-close-overlay"></div>

        <div x-show="isOpen" x-trap.inert.noscroll="isOpen"
            @if (filled($id)) x-on:keydown.window.escape="$dispatch('{{ $closeEventName }}', { id: '{{ $id }}' })"
            @else
                x-on:keydown.window.escape="isOpen = false" @endif
            ...
            <div @class([
                'w-full mx-auto p-2 space-y-2 bg-white cursor-default pointer-events-auto filament-modal-window',
                ...
                <div class="space-y-2 filament-modal-content">
                    @if ($heading || $subheading)
                        <div @class(['p-4 space-y-2 text-center', 'dark:text-white' => $darkMode])>
                            @if ($heading)
