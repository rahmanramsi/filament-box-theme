...
$buttonClasses = array_merge(
    ['inline-flex items-center justify-center gap-1 font-medium border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset filament-button', 'dark:focus:ring-offset-0' => $darkMode, 'opacity-70 cursor-not-allowed' => $disabled, 'h-9 px-4 text-sm' => $size === 'md', 'h-8 px-3 text-sm' => $size === 'sm', 'h-11 px-6 text-lg' => $size === 'lg'],
    $outlined
        ? [
...
$iconClasses = \Illuminate\Support\Arr::toCssClasses(['filament-button-icon', 'w-5 h-5' => $size === 'md', 'w-4 h-4' => $size === 'sm', 'w-6 h-6' => $size === 'lg', 'mr-1 -ml-2 rtl:ml-1 rtl:-mr-2' => $iconPosition === 'before' && $size === 'md', 'mr-2 -ml-3 rtl:ml-2 rtl:-mr-3' => $iconPosition === 'before' && $size === 'lg', 'mr-1 -ml-1.5 rtl:ml-1 rtl:-mr-1.5' => $iconPosition === 'before' && $size === 'sm', 'ml-1 -mr-2 rtl:mr-1 rtl:-ml-2' => $iconPosition === 'after' && $size === 'md', 'ml-2 -mr-3 rtl:mr-2 rtl:-ml-3' => $iconPosition === 'after' && $size === 'lg', 'ml-1 -mr-1.5 rtl:mr-1 rtl:-ml-1.5' => $iconPosition === 'after' && $size === 'sm']);
...
@if ($tag === 'button')
    <button @if ($keyBindings) x-mousetrap.global.{{ implode('.', $keyBindings) }} @endif
        @if ($tooltip) x-tooltip.raw="{{ $tooltip }}" @endif type="{{ $type }}"
        wire:loading.attr.delay="disabled" {!! $hasLoadingIndicator ? 'wire:loading.class.delay="opacity-70 cursor-wait"' : '' !!} {!! $hasLoadingIndicator && $loadingIndicatorTarget ? "wire:target=\"{$loadingIndicatorTarget}\"" : '' !!} {!! $disabled ? 'disabled' : '' !!}
        x-data="{
            form: null,
            label: {{ \Illuminate\Support\Js::from($slot->toHtml()) }},
            isUploadingFile: false,
        }" x-bind:disabled="isUploadingFile"
        x-bind:class="{ 'opacity-70 cursor-wait': isUploadingFile }" x-init="form = $el.closest('form')
        ...
        @elseif ($hasLoadingIndicator)
            <x-filament-support::loading-indicator wire:loading.delay x-cloak :wire:target="$loadingIndicatorTarget"
                :class="$iconClasses" />
        @endif

        <span class="flex items-center gap-1">
            @if ($type === 'submit' && filled($form))
                <x-filament-support::loading-indicator x-show="isUploadingFile" x-cloak :class="$iconClasses" />
...
@elseif ($tag === 'a')
    <a @if ($keyBindings || $tooltip) x-data="{}" @endif
        @if ($keyBindings) x-mousetrap.global.{{ implode('.', $keyBindings) }} @endif
